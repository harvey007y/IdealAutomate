...
namespace System.Windows.Forms.Samples {
    public partial class MoveTo : Form {
        ...
        DataGridViewExt dgvResults = new DataGridViewExt("MoveToResults");       
        static StringBuilder _MoveToErrors = new StringBuilder();
        public static string strPathToMoveTo = @"C:\SVNIA\trunk";
        public static string strMoveToPattern = @"*.*";

        public static string strMoveToExcludePattern = @"*.dll;*.exe;*.png;*.xml;*.cache;*.sln;*.suo;*.pdb;*.csproj;*.deploy";

        public static string strMoveToText = @"notepad";

        public static string strLowerCaseMoveToText = @"notepad";
        ...
        private async void TabControl1_MouseClick(object sender, MouseEventArgs e) {
            ...
            myActions.SetValueByKey("CurrentIndexMoveTo", _CurrentIndex.ToString());
           ...
           // Methods myActions = new Methods();
            ...
            myActions.SetValueByKey("CurrentIndexMoveTo", tabControl1.SelectedIndex.ToString());
 
                _newTab = true;
 
            string strInitialDirectory = "MoveToResults";
        ...
        private async void RefreshDataGrid() {
           ...
           // Methods myActions = new Methods();
            ...
            strPathToMoveTo = strFolderToUse;
            strMoveToPattern = strFileTypeToUse;
            strMoveToExcludePattern = strExcludeToUse;
 ...
 //               myActions.MessageBoxShow(damageResult);
        ...
        private void MoveTo_ClickAsync(object sender, EventArgs e) {
            _MoveToErrors.Length = 0;
            Methods myActions = new Methods();
            myActions = new Methods();  
            
            try {
 //               var damageResult =  MoveToTask();
 //               progressBar1.BeginInvoke(
 //        new Action(() => {
 //            progressBar1.Value = 0;
 //        }
 ...
 //               this.WindowState = FormWindowState.Maximized;
        ...
        private void startBtn_Click(object sender, EventArgs e) {
            ...
            string strFullFileName = myActions.GetValueByKey("MoveFromVar");
            int intLastSlash = strFullFileName.LastIndexOf('\\');
            if (intLastSlash < 1)
            {
                myActions.MessageBoxShow("Could not find last slash in Move From filename - aborting");
                return;
            }
        ...
        private void backgroundWorker1_DoWork(object sender, DoWorkEventArgs e) {
            ...
            if (File.Exists(strPathToMoveTo)) {
                System.IO.FileInfo fi = new System.IO.FileInfo(strPathToMoveTo);
                myFileList.Add(fi);

            } else {
                myFileList = TraverseTree(strMoveToPattern, strPathToMoveTo);
            }
            int intFiles = 0;
            ...
            Parallel.ForEach(myFileList, myFileInfo => {
                ...
                if (backgroundWorker1.CancellationPending) {
                    ...
                    if (currentPercent > prevPercent + 10) {
                        ...
                        if (_MoveToErrors.Length > 0) {
                            myResult += "\n\rErrors: " + _MoveToErrors.ToString();
                        }
     ...
     Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData) + "\\IdealAutomate\\" + Program.MyRoamingFolder;
            using (FileStream fs = new FileStream(settingsDirectory + @"\MatchInfo.txt", FileMode.Create)) {
                ...
                file.WriteLine(@"-- " + strMoveToText + " in " + strPathToMoveTo + " from " + strMoveToPattern + " excl  " + strMoveToExcludePattern + " --");
                foreach (var item in matchInfoList) {
        ...
        public  List<FileInfo> TraverseTree(string filterPattern, string root) {
            string[] arrayExclusionPatterns = strMoveToExcludePattern.Split(';');
        ...
        public static void ReadFileToString(string fullFilePath, int intLineCtr, List<MatchInfo> matchInfoList) {
            while (true) {
                try {
                    if (fullFilePath.EndsWith(".odt")
                        ...
                        if (FindTextInWordPad(strMoveToText, fullFilePath)) {
                            intHits++;
                            ...
                            myMatchInfo.LineText = strMoveToText;
                            matchInfoList.Add(myMatchInfo);
                        }
                    }
            ...
            fullFilePath.EndsWith(".docx")
                        ...
                        if (FindTextInWord(strMoveToText, fullFilePath)) {
                            intHits++;
                            boolStringFoundInFile = true;
                            MatchInfo myMatchInfo = new MatchInfo();
                            myMatchInfo.FullName = fullFilePath;
                    ...
                    using (FileStream fs = new FileStream(fullFilePath, FileMode.Open)) {
                        using (StreamReader sr = new StreamReader(fs, Encoding.Default)) {
                            ...
                            while ((s = sr.ReadLine()) != null) {
                                ...
                                if (boolUseRegularExpression) {
                                    if (boolMatchCase) {
                                        if (System.Text.RegularExpressions.Regex.IsMatch(s, strMoveToText, System.Text.RegularExpressions.RegexOptions.None)) {
                                            intHits++;
                                            boolStringFoundInFile = true;
                                            MatchInfo myMatchInfo = new MatchInfo();
                                            myMatchInfo.FullName = fullFilePath;
                                            myMatchInfo.LineNumber = intLineCtr;
                                            myMatchInfo.LinePosition = s.IndexOf(strMoveToText) + 1;
                                            myMatchInfo.LineText = s;
                                            matchInfoList.Add(myMatchInfo);
                                        }
                                    } else {
                                        s_lower = s.ToLower();
                                        if (System.Text.RegularExpressions.Regex.IsMatch(s_lower, strLowerCaseMoveToText, System.Text.RegularExpressions.RegexOptions.IgnoreCase)) {
                                            intHits++;
                                            boolStringFoundInFile = true;
                                            MatchInfo myMatchInfo = new MatchInfo();
                                            myMatchInfo.FullName = fullFilePath;
                                            myMatchInfo.LineNumber = intLineCtr;
                                            myMatchInfo.LinePosition = s_lower.IndexOf(strLowerCaseMoveToText) + 1;
                                            myMatchInfo.LineText = s;
                                            matchInfoList.Add(myMatchInfo);
                                        }
                                    }
                                    ...
                                    if (boolMatchCase) {
                                        ...
                                        s_lower = s.ToLower();
                                        if (s_lower.Contains(strLowerCaseMoveToText)) {
        ...
        protected static bool FindTextInWord(string text, string flname) {
            ...
            try {
                ...
                if (ex.InnerException != null) {
                    _MoveToErrors.AppendLine(ex.InnerException.ToString() + " filename is:" + flname + " - Line 5733 in ExplorerView");
                } else {
                    _MoveToErrors.AppendLine(ex.Message + " filename is:" + flname + " - Line 5735 in ExplorerView");
                }
            }
     ...
     Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData) + "\\IdealAutomate\\" + Program.MyRoamingFolder;
            ...
            for (int i = 0; i < numOfTabs - 1; i++) {
                ...
                tabControl1.TabPages[i].ToolTipText = "Click on this tab to MoveTo in " + _dir.FileView.FullName;
                _CurrentIndex = i;
           ...
           AddDataGridToLastTab();
            ...
            _CurrentIndex = myActions.GetValueByKeyAsInt("CurrentIndexMoveTo");
          ...
          //  myActions.SetValueByKey("CurrentIndexMoveTo", tabControl1.SelectedIndex.ToString());
            ...
            strInitialDirectory = "MoveToResults";
            new DgvFilterManager(dgvResults);
            int sortedColumn = myActions.GetValueByKeyAsInt("SortedColumn_" + strInitialDirectory.Replace(":", "+").Replace("\\", "-"));
            string myDirection = myActions.GetValueByKey("SortOrder_" + strInitialDirectory.Replace(":", "+").Replace("\\", "-"));
        ...
        private void AddDataGridToLastTab() {
          ...
          //  tabControl1.TabPages.Insert(_CurrentIndex + 1, "MoveTo Folder or File");
        //    tabControl1.TabPages[_CurrentIndex + 1].Text = _dir.FileView.Name;
            tabControl1.TabPages[_CurrentIndex].ToolTipText = "Click on this tab to MoveTo in " + _dir.FileView.FullName;
            DataGridViewExt dgvResults = new DataGridViewExt("MoveToResults");
        ...
        private void AddDataGridToTab(string pstrInitialDirectory) {
            tabControl1.TabPages.Insert(_CurrentIndex + 1, "** MoveTo A Folder **");
            DataGridViewExt dgvResults = new DataGridViewExt("MoveToResults");
        ...
        private void btnFolder_Click(object sender, EventArgs e) {
         ...
         //   dialog.ShowDialog();
        //    dialog.SelectedPath = myActions.GetValueByKey("LastMoveToFolder");
            dialog.SelectedPath = tabControl1.TabPages[_CurrentIndex].ToolTipText.Replace("Click on this tab to MoveTo in ", "");
            string str = "LastMoveToFolder";
            ...
            if (result == System.Windows.Forms.DialogResult.OK && (Directory.Exists(dialog.SelectedPath) || File.Exists(dialog.SelectedPath))) {
                ...
                myActions.SetValueByKey("LastMoveToFolder", dialog.SelectedPath);
   ...
   Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData) + "\\IdealAutomate\\" + Program.MyRoamingFolder;
            ...
            _CurrentIndex = tabControl1.SelectedIndex;
            myActions.SetValueByKey("CurrentIndexMoveTo", _CurrentIndex.ToString());
            tabControl1.TabPages[tabControl1.TabPages.Count - 1].ToolTipText = "Click on this tab to MoveTo in " + ((ComboBoxPair)(cbxFolder.SelectedItem))._Value;
        }
      ...
      Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData) + "\\IdealAutomate\\" + Program.MyRoamingFolder;
        ...
        private void cbxFolder_Leave(object sender, EventArgs e) {            
            ...
            _CurrentIndex = tabControl1.SelectedIndex;
            myActions.SetValueByKey("CurrentIndexMoveTo", _CurrentIndex.ToString());
            tabControl1.TabPages[tabControl1.TabPages.Count - 1].ToolTipText = "Click on this tab to MoveTo in " + cbxFolder.Text;
        }
        ...
        private void btnOpenInIAE_Click(object sender, EventArgs e) {
            ...
            if (_CurrentIndex == numOfTabs - 1) {
                myActions.MessageBoxShow("You can only open files in Ideal Automate Explorer when you are not in the **MoveTo A Folder** tab");
                return;
            }
        ...
        internal void MoveTo_Click(object p1, object p2) {
