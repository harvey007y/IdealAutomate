...
namespace VisualStudioHotKeys {
    ...
    public partial class MainWindow : Window {
            ...
            var window = new Window() //make sure the window is invisible
      {
            ...
            if (strWindowTitle.StartsWith("VisualStudioHotKeys")) {
            ...
            XmlElement node = doc.CreateElement("FileDigestAlgorithm", root.NamespaceURI);
            node.InnerText = "SHA-256";
            //add to elements collection
            root.AppendChild(node);

            node = doc.CreateElement("FileDigestValue", root.NamespaceURI);
            node.InnerText = hashData;

            //add to elements collection
            root.AppendChild(node);


            hashInput = String.Format("{0}{1}", doc, paramFileSha256KeyHash);
            hashData = GtHash.HashSHA256(hashInput);

            // remove extension from mainFileName
            string fileName = fileParms.mainFileName;
            int fileExtPos = fileName.LastIndexOf(".");
            if (fileExtPos >= 0) {
                fileName = fileName.Substring(0, fileExtPos);
            }
            File.WriteAllText(swiftFilePath + fileParms.mainFileName + "par.lau", hashData);
            File.WriteAllBytes(swiftFilePath + fileParms.paramFileName, Encoding.ASCII.GetBytes(doc.OuterXml));
