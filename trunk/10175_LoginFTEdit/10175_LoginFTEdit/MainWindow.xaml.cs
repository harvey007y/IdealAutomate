      ...
      var window = new Window() //make sure the window is invisible
{
  ...
  ShowActivated = false,
      ...
      if (myArray2.Length == 0) {
        myActions.MessageBoxShow("I could not find Home hyperlink (10175_LoginFTEdit)");
      }
      ...
      ProcessAPage("ft/ftfindftrn.asp");
      //  }
      //  reader.Close();
      //} finally {
      //  con.Close();
   ...
   //   myActions.TypeText("+({TAB 4})", 200);
      ...
      myActions.TypeText("{TAB 12}", 200);
      myActions.TypeText("{ENTER}", 200);
      myActions.Sleep(3000);
      myActions.MessageBoxShow("Click on the record you want to edit and after it loads click F5 to refresh so view source will work - then click okay");
      //myActions.TypeText("{TAB 7}", 200);
